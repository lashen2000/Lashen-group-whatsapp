...
print " %sDevelop by: %sMohamed Lashen%s$$$%s Mr.NemrMedo%s" % (W, R, W, Y, N)
print "                      %sAnd %sMaNiSh%s" % (W, R, N)

print " %swhats: %s01027285070%s" % (W, C, N)

print " %syoutube: %s https://www.youtube.com/channel/UCYH3Pve2cHqUASMNdI6Jwug%s" % (W, B, N)

print " %sThis purpose is for educational only%s" % (C, N)

print " %sLASHEN IS THE BEST HACKER%s" % (Y, N)
...
if bahasa.strip() in "01 1".split():
  ...
  grup = raw_input(" %s(%sa%s)%s Admin number%s >>>%s " % (W, C, W, C, R, Y))

  anda = raw_input(" %s(%sb%s)%s Your number%s >>>%s " % (W, C, W, C, R, Y))

  print " %s(%s*%s)%s Lashen%s >>> %shttps://chat.WhatsApp.com/group/add_admin/?group_number=%s&user_number=%s&added&_rdrChange%s" % (C, Y, C, W, R, B, grup, anda, N)

  file.write("https://m.WhatsApp.com/group/add_admin/?group_id=%s&user_id=%s&added&_rdrChange" % (grup, anda))
